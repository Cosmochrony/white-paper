\subsection{Entanglement as a Sub-Resolution Event in Reprojection Dynamics}
  ...
  \paragraph{Direct and anti-correlated entanglement.}
    ...
    For the conjugate Weil pair that organizes the spectral cascade, the singlet
    case is a derived quantity rather than a qualitative picture: after removal
    of the zero mode the minimal admissible fiber is the two-dimensional
    Born--Infeld parity pair, identified with the spin-$\tfrac{1}{2}$
    representation, and the singlet invariant it carries has entanglement entropy
    exactly $\log 2$ --- the value behind the singlet correlator
    $E(\hat{a},\hat{b}) = -\hat{a}\cdot\hat{b}$ and the Tsirelson bound.
  ...
  \paragraph{Summary.}
    ...
    Entanglement is a local sub-resolution event in the reprojection dynamics
    --- the regime in which the joint fiber is non-decomposable and its balance
    invariant lies below the projective resolution; it requires no special
    configuration of $\chi$, no modification of $\Pi$, and no non-local
    dynamical influence.
