\subsection{Simulation Algorithms for \texorpdfstring{$\chi$}{χ}-Field Dynamics}
  ...
  \paragraph{Projective update rule.}
    ...
    \end{equation}
    This enforces strict monotonicity, a universal upper bound on the local
    admissible projective rate, and suppression of gradient-driven
    instabilities.

  \paragraph{Reference pseudocode.}
    \begin{algorithm}[t]
\caption{Bounded $\chi$-projection with diagnostics}
\label{alg:bounded-chi-relaxation}
\begin{algorithmic}[1]
  \Require $\chi^{(0)}_i$, $K_{ij}$, bound $c$,
  tolerances $\varepsilon_\chi,\varepsilon_S$
  \Ensure Projected $\chi^\star$ and diagnostics
...
\end{algorithmic}
  ...
  \paragraph{Localized configurations.}
    ...
    configurations that saturate local projective capacity, interpreted as
    numerical counterparts of solitonic excitations
