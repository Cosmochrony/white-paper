\subsection{Emergence of the Schr\"{o}dinger equation as an effective projected dynamics}
  ...
  \subsubsection*{Main theorem}
    ...
    \begin{theorem}[Emergence of quantum dynamics from projected admissibility structure]
      ...
      \begin{itemize}
        ...
        admissibility operator $\mathcal{R}$,
        ...
        $A_n(\tau) \leq c_{\mathrm{BI}}/\sqrt{\lambda_n}$ at all $\tau$,
    ...
    but to make explicit how the effective Schr\"odinger dynamics follows from the
    projected admissibility structure of the substrate in the weak-amplitude regime.
    ...
    \paragraph{Step 1: Admissibility structure and linearization.}

      The admissibility structure of $\Pi$ induces, along the projective ordering,
      a relation of the generic form
      \[
        \partial_{\tau}\chi = \mathcal{R}[\chi],
      \]
      where $\tau$ denotes the projective ordering parameter and $\mathcal{R}$ is the
      nonlinear admissibility operator encoding the constraints of~$\Pi$ and the
      Born--Infeld bound.
      The label ``$\mathcal{R}$'' is retained for continuity with the formal literature;
      it does not imply that $\chi$ undergoes an independent dynamical relaxation.
      The operator $\mathcal{R}$ encodes admissibility constraints, not a temporal
      evolution of the static substrate~$\chi$.
      A configuration $\bar\chi$ is a fixed point of $\mathcal{R}$ if and only if it is
      admissible; small perturbations $\delta\chi = \chi - \bar\chi$ then evolve as
      \[
        \partial_\tau\,\delta\chi = \mathcal{L}\,\delta\chi + O(\delta\chi^2),
        \qquad \mathcal{L} \equiv D\mathcal{R}\big|_{\bar\chi}.
      \]

    \paragraph{Step 2: Spectral decomposition.}

      Hypothesis~(H5) asserts that the low-lying modes of $\mathcal{L}$ are
      diagonalizable within the projected-stable sector.
      Let $\{u_n\}$ be a basis of eigenvectors with eigenvalues $\{\mu_n\}$:
      $\mathcal{L}\,u_n = \mu_n\,u_n$.
      Decompose the perturbation as $\delta\chi = \sum_n c_n(\tau)\,u_n$; each
      coefficient then satisfies
      \[
        \partial_\tau c_n = \mu_n\,c_n,
        \qquad c_n(\tau) = c_n(0)\,e^{\mu_n\tau}.
      \]

    \paragraph{Step 3: Projection constraints fix $\mathrm{Re}(\mu_n) = 0$.}

      Two independent structural constraints act on the eigenvalues.

      \emph{Born--Infeld bound~(H3).}
      The amplitude bound $A_n(\tau) \leq c_{\mathrm{BI}}/\sqrt{\lambda_n}$ excludes
      $\operatorname{Re}(\mu_n) > 0$: a growing mode would eventually violate the
      admissibility envelope and exit the domain of validity of the linearized dynamics.

      \emph{Finite projective resolution~(H2).}
      A mode with $\operatorname{Re}(\mu_n) < 0$ has amplitude decaying as
      $|c_n(\tau)| = |c_n(0)|\,e^{\operatorname{Re}(\mu_n)\tau} \to 0$.
    ...
    \paragraph{Step 5: Reconstruction of the wavefunction and the operator equation.}
      ...
      Passing from the projective ordering parameter $\tau$ to the effective projected time
    ...
    \paragraph{Structural conclusions.}
      ...
      Fifth, the Schr\"{o}dinger equation is the continuous limit of a discrete admissible
      cascade.
      The derivation above recovers unitary dynamics in the continuous-parameter regime.
      The underlying structure is however discrete: the admissible transitions
      $\Pi_0 \to \Pi_1 \to \cdots$ form an oriented state machine
      (Section~\ref{subsec:monotonicity-and-arrow-of-time}) whose Mosco limit~\cite{Beau2026c}
      yields the continuous-time equation~(C3) in the regime where successive transitions
      are densely packed and the discreteness of the BFS depth~$n$ becomes unresolvable
      at the effective scale.
      The time parameter~$t$ is therefore the effective image of the path index~$n$, and
      the Hamiltonian~$\hat{H}_{\mathrm{eff}}$ is the generator of Mosco-limit transport
      on the admissibility graph.
      The quantum-to-classical transition corresponds to the regime where the graph
      becomes locally tree-like and branching weights become negligible.
