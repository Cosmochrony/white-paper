\subsection{Emergence of the Schr\"{o}dinger equation as an effective projected dynamics}
  ...
  \subsubsection*{Main theorem}
    ...
    Steps~1--5 and Appendix~B.\ref{app:schrodinger-hamiltonian} constitute its proof.

    \begin{theorem}[Emergence of quantum dynamics from projected substrate relaxation]
      \label{thm:schrodinger-emergence}
      Let $(\Omega, \mathcal{R}, \Pi)$ be a BAT triple consisting of:
      \begin{itemize}
      ...
      \begin{enumerate}
        ...
        For each mode $u_n$, the unique global solution of the projected dynamics is
        $c_n(\tau) = c_n(0)\,e^{-i\omega_n\tau}$,
        giving the modal equation
        \[
        ...
        The projected wavefunction $\psi(x,t) = \sum_n a_n(t)\,\phi_n(x)$ satisfies
        \[
          i\hbar_{\mathrm{eff}}\,\partial_t\psi
          = \hat{H}_{\mathrm{eff}}\,\psi + \mathcal{N}_{\Pi,\mathrm{BI}}[\psi],
        \]
        where $\hat{H}_{\mathrm{eff}}$ is Hermitian on the projected-stable Hilbert space, defined spectrally by
        ...
        It holds if and only if (H2) and (H3) hold simultaneously on the projected-stable sector:
        \[
          \textup{unitarity}
          \;\Longleftrightarrow\;
    ...
    \begin{proof}
      ...
      (C2) follows by direct integration of $\partial_\tau c_n = -i\omega_n c_n$ (unique
      solution of a linear ODE with constant coefficients).
      Multiplication by $\hbar_{\mathrm{eff}}$ and the energy-frequency relation
      $E_n = \hbar_{\mathrm{eff}}\omega_n$ (Section~\ref{subsec:energy-frequency-solitons})
      give the modal equation.

      (C3) follows from (C2) by reconstruction: $\psi = \sum_n a_n\phi_n$, and
      $i\hbar_{\mathrm{eff}}\partial_t\psi = \sum_n E_n a_n\phi_n = \hat{H}_{\mathrm{eff}}\psi$
      at leading order.
      The correction $\mathcal{N}_{\Pi,\mathrm{BI}}$ collects Born--Infeld nonlinearities
      $O(A_n/A_n^{\max})$ from (H3) and inter-modal coupling
      $O(\partial_\tau a_n) = O(a_n^2, \nabla a_n, \Pi_{\mathrm{sat}})$ from (H4).
      Both are negligible under (H4).

      (C4) follows from the spectral identification in
      Appendix~B.\ref{app:schrodinger-hamiltonian}:
      $\omega_n^2 = \lambda_n$ at leading order (from the admissibility envelope~\cite{Beau2026a1}),
      combined with the operator--metric correspondence
      $\sigma_2(-\Delta_\chi)(x,k) = g^{\mu\nu}k_\mu k_\nu$~\cite{Beau2026c},
      and the non-relativistic expansion
      $\hbar_{\mathrm{eff}}\sqrt{-\Delta_g} \simeq m_{\mathrm{eff}}c^2 - \hbar_{\mathrm{eff}}^2\Delta_g/(2m_{\mathrm{eff}})$.
      ...
      sector, breaking norm preservation.
    \end{proof}
