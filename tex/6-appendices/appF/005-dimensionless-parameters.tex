\subsection{Dimensionless Parameters}
  ...
  \paragraph{\texorpdfstring{$\Omega_\chi$}{Ωχ} (Projective capacity parameter).}
    A dimensionless global quantity characterizing the fraction of total admissible
    projective capacity stored in spatial gradients.
    In cosmological regimes, it plays a role analogous to a density parameter.

  \paragraph[$F$]{Frustration density (operational proxy).}
    $F$ denotes a dimensionless proxy for the density of incompatible local fiber matching constraints in the fiber
    sector.
  ...
  \paragraph[$r_F$]{Relative frustration ratio.}
    ...
    structure.
    In particular, $r_F$ organizes the expected transition between distinct symmetry channels in the superconducting
    regime.
