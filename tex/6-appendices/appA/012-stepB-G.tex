\subsection{Prediction: $G_{\mathrm{eff}}$ in terms of $c_{\mathrm{BI}}$ and $\hbar_\chi$}
\label{subsec:stepB-G}

Comparing~\eqref{eq:BI-IR} with the standard Einstein--Hilbert normalization
$\frac{c^4}{16\pi G} \int R \sqrt{-g}\, d^4x$ and using $c=c_{\mathrm{BI}}$, the identification
requires:
\[
  \frac{c_{\mathrm{BI}}^4}{16\pi G_{\mathrm{eff}}} = \frac{c_{\mathrm{BI}}^2}{2\hbar_\chi}.
\]
Formally, this gives:
\begin{equation}
  G_{\mathrm{eff}} = \frac{c_{\mathrm{BI}}^2 \hbar_\chi}{8\pi}.
  \label{eq:G-prediction}
\end{equation}

A dimensional check in SI units yields $[c_{\mathrm{BI}}^2 \hbar_\chi] = \mathrm{kg}\,\mathrm{m}^4\,\mathrm{s}^{-3}$, while
$[G]=\mathrm{m}^3\,\mathrm{kg}^{-1}\,\mathrm{s}^{-2}$.
These do not match, which signals that the prefactor $c_{\mathrm{BI}}^4/\hbar_\chi^2$ in
...
The structural prediction $G_{\mathrm{eff}} = G(c_{\mathrm{BI}}, \hbar_\chi)$ is robust.
