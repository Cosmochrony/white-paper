\subsection{Spectrum of \(\chi\)-Field Fluctuations and CMB Anisotropies}
  ...
  \subsubsection{Scale-Dependent Attenuation of Low-$\ell$ Modes}
    \label{app:lowell_attenuation}

    Beyond the averaged suppression shown in the main text, Cosmochrony suggests a more general structural
    effect at the largest angular scales. In particular, a scale-dependent attenuation acting on global modes
    naturally preserves the relative ordering of low-$\ell$ multipoles while reducing their overall amplitude
    as $\ell \rightarrow 0$.

    Figure~\ref{fig:cmb_lowell_unsmoothed} illustrates this effect by applying a monotonic attenuation factor
    to a multipole pattern, while leaving the relative variations between neighboring modes unchanged. This
    construction is intended to highlight a qualitative difference with $\Lambda$CDM, in which low-$\ell$
    features arise from stochastic fluctuations rather than deterministic modulation.

    \begin{figure}[t]
      \centering
      \includegraphics[width=\linewidth]{C-appendix-cosmo/cmb_lowell_unsmoothed}
      \caption{
        Illustrative comparison between the Planck 2018 low-$\ell$ CMB temperature spectrum (points), the
        $\Lambda$CDM ensemble-averaged prediction (dashed line), and an unsmoothed Cosmochrony-inspired attenuation
        applied to a multipole pattern. The Cosmochrony curve preserves the relative ordering of low-$\ell$
        multipoles while introducing a scale-dependent attenuation that increases toward the largest angular
        scales. This construction is illustrative and does not represent an independent multipole-level
        prediction.
      }
  ...
  It is important to stress that this unsmoothed representation does not constitute an independent
    multipole-by-multipole prediction. Rather, it serves as an illustrative example of how global constraints
    on the $\chi$ field could manifest as a scale-dependent compression of large-angle temperature
    anisotropies, without erasing their underlying structure.
