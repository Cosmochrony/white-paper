...
\section{Numerical Methods and Technical Supplements}
  ...
  \paragraph{Availability and status of the simulation code.}
    The numerical methods described in this appendix are implemented in an open-source
    simulation codebase developed to explore effective relaxation regimes of the
    \(\chi\) substrate.
    The code is archived on Zenodo under a persistent digital object identifier (DOI),
    ensuring long-term accessibility and citability:
    \begin{quote}
      \emph{Cosmochrony Simulation Code}, Zenodo DOI:
      \href{https://doi.org/10.5281/zenodo.18292335}{10.5281/zenodo.18292335}
    \end{quote}

    This implementation is provided as a computational support to the present work.
    It does not constitute a discretization of the fundamental \(\chi\) substrate, nor
    does it introduce additional physical assumptions beyond those stated in the main text.
    The numerical schemes employed are effective representations designed for
    exploratory and illustrative purposes, and alternative implementations respecting
    the same relaxation constraints and kinematic bounds are equally admissible.

    The archived DOI refers to a continuously evolving codebase.
    Specific numerical results or figures discussed in this work may correspond to
    particular tagged releases of the repository, which are individually versioned and
    traceable through Zenodo.
  ...
  \paragraph{Scope of the appendix.}
    ...
    \begin{itemize}
      ...
      in the main text and in Appendices~\ref{sec:appendix-conceptual} and~\ref{sec:appendix-cosmo}.
    \end{itemize}

    The emphasis throughout is on internal consistency, numerical stability, and
    faithful implementation of the theoretical principles articulated in the main body of the work.
    No claim is made that the numerical results obtained using these methods are unique or exhaustive.

  \paragraph{Interpretation of numerical results.}
    Numerical simulations presented or discussed in this appendix should be regarded as exploratory.
    They are intended to test qualitative mechanisms—such as relaxation-driven
    emergence of geometry, solitonic stability, or scaling relations—rather than to produce precision predictions.

    Where numerical values are quoted, they serve as order-of-magnitude indicators or illustrative examples.
  ...
  \paragraph{Relation to the main text.}
    ...
    \begin{itemize}
      ...
      in Appendix~\ref{sec:appendix-cosmo}.
    \end{itemize}
    ...
    \input{D-appendix-simulation/D01-collective_coupling}
    \input{D-appendix-simulation/D02-chi-parameters}
    \input{D-appendix-simulation/D03-consistency_checks}
    \input{D-appendix-simulation/D04-simulation-algorithms-for-chi-field-dynamics}
    \input{D-appendix-simulation/D05-numerical-validation-of-the-chi-rightarrow-chi_eff-transition}
    \input{D-appendix-simulation/D06-renormalization-hbar}
    \input{D-appendix-simulation/D07-spectral_ratio_derivation}
    \input{D-appendix-simulation/D08-rotation-curve-fits}
