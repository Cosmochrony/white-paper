\subsection{Effective Lagrangian Description of the $\chi$\-Field Dynamics}
  \label{subsec:effective-lagrangian-description-of-the-chi-field-dynamics}

  \textit{Note on the status of the effective action: The comprehensive Lagrangian density presented in this appendix is
  not to be interpreted as a fundamental axiomatic starting point.
  In the Cosmochrony framework, the only fundamental dynamics resides in the irreversible relaxation of the $\chi$ field
  as described in the main body of this article. The standard Einstein-Dirac-Maxwell action, including potential
  extensions like torsion, is presented here as a \textbf{reconstruction a posteriori}. It serves as a proof of concept
  to demonstrate how the emergent degrees of freedom of the $\chi$ field—specifically its localized solitonic
  excitations and their geometric coupling—map onto the effective field theories of the Standard Model and General
  Relativity.}
  ...
  \paragraph{Effective action formulation.}
    In regimes where $\chi$ admits a quasi-stable geometric interpretation, the dynamics may be encoded in an
    effective action\footnote{This action is intended as an effective description valid in regimes where spacetime emerges,
  not as a fundamental assumption of a pre-existing four-dimensional manifold}:
    \begin{equation}
...
\subsubsection{Gravity and Time: The Geometric Relaxation Term ($\mathcal{L}_{\text{Gravity/Time}}$)}
  ...
  \[\mathcal{L}_{\text{Gravity/Time}} = \frac{1}{16\pi G_{\text{eff}}} \cdot F(\chi) R - \Lambda_{\text{Flow}}^4
  \cdot \chi\]

  \begin{itemize}
    \item $R$ is the Ricci scalar (or an effective curvature scalar incorporating Torsion).
    \item $F(\chi)$
    is the non-minimal coupling function, required to recover the Einstein-Hilbert action in the asymptotic limit
    $\chi \to \chi_0$.
    \item The effective gravitational constant is $G_{\text{eff}} \propto [F(\chi)]^{-1}$.
    \item The term $-\Lambda_{\text{Flow}}^4 \cdot \chi$
    introduces the driving force for expansion (the ``cosmochrony flow''). $\Lambda_{\text{Flow}}$
    is a constant with dimensions of mass, ensuring that $\chi$ is continuously driven towards larger values (
    $\partial_t \chi \ge 0$), intrinsically linking time and geometric growth.
  \end{itemize}

\subsubsection{Field Dynamics: The Soliton Generation Term ($\mathcal{L}_{\chi/\text{Soliton}}$)}

  This component governs the dynamics of the field $\chi$
  itself, including the non-linearity required for particle formation.
  ...
  \begin{itemize}
    ...
    \item $V_{\text{Soliton}}(\chi)$ is a non-linear potential (e.g.,
    $V \propto \mu \chi^2 - \lambda \chi^4 + \kappa \chi^6 + \ldots$
    ) necessary to guarantee the existence and stability of localized, finite-energy solutions (solitons) that are
    interpreted as elementary particles.
  \end{itemize}

  As an example, for the $\phi^4$-like potential $V(\chi) = \lambda (\chi^2 - 1)^2$
  , the static soliton solution is $\chi(x) = \tanh(x / \xi)$, where $\xi = \sqrt{2 / \lambda}$
  sets the soliton width.
  The energy of this configuration is $E = \left(4 \sqrt{2} / 3\right) \sqrt{\lambda}$, identifying the soliton mass $m \propto \sqrt{\lambda}$.

\subsubsection{Emergent Forces and Matter ($\mathcal{L}_{\text{Forces/Matter}}$)}

  This encompasses the emergent field theories: electromagnetism (photons, $A_\mu$
  ) and fermionic matter (electrons, $\Psi$) which are understood as dynamic excitations of the $\chi$-field.
  ...
  \begin{itemize}
    ...
    -fluctuations.
    \item $\mathcal{L}_{\text{Dirac}}^{\text{Torsion}}$ is the emergent Lagrangian for fermionic matter fields
    $\Psi$. Crucially, it must be formulated in an **affine manifold with Torsion $T$** (where $T$
...
\subsection{Nature of the $\chi$ Field}\label{subsec:nature-of-the-$chi$-field}

The field $\chi(x^\mu)$
is postulated as a real scalar field defined on a four-dimensional differentiable manifold.
Unlike conventional scalar fields in quantum field theory, $\chi$
does not represent a matter degree of freedom propagating \emph{within} spacetime.
Instead, it encodes the local geometric scale governing the emergence of spacetime itself.

Operationally, $\chi$
may be interpreted as a proper wavelength field whose monotonic increase defines both spatial separation and
temporal flow.
...
\subsection{Effective Evolution Equation}\label{subsec:effective-evolution-equation}
...
\subsection{Bootstrap Self-Consistency and Seed-Independence}
\label{sec:bootstrap}

To demonstrate that the effective metric $g_{\mu\nu}^{\text{phys}}$ genuinely emerges from $\chi$ dynamics rather than being a hidden input, we outline a bootstrap verification procedure.

\paragraph{Bootstrap algorithm.}
  \begin{enumerate}
    \item \textbf{Initialize:} Choose an arbitrary seed metric $g_{\mu\nu}^{(0)}$ (e.g., Minkowski $\eta_{\mu\nu}$) purely for calculational purposes.

    \item \textbf{Solve for $\chi$:} Given $g^{(0)}$, solve the $\chi$-field dynamics:
    \begin{equation}
      \nabla^{(0)}_\mu \nabla^{(0)\mu}\chi - V'(\chi) = S[\chi,\rho]
    \end{equation}

    \item \textbf{Compute emergent metric:} From the solution $\chi^{(1)}$, compute:
    \begin{equation}
      g_{\mu\nu}^{(1)} = \frac{\delta^2 S[\chi^{(1)}]}{\delta(\nabla_\mu\chi)\delta(\nabla_\nu\chi)}
    \end{equation}
    or via phenomenological relations:
    \begin{equation}
      g_{00}^{(1)} = -\left(\frac{\partial_t\chi^{(1)}}{c}\right)^2, \quad
      g_{ij}^{(1)} = \left(\frac{\chi^{(1)}}{\chi_0}\right)^2\delta_{ij}
    \end{equation}

    \item \textbf{Iterate:} Use $g^{(1)}$ as input to step 2, generating $\chi^{(2)} \to g^{(2)} \to \cdots$

    \item \textbf{Convergence:} The sequence $\{g^{(n)}\}$ must converge: $g^{(n)} \to g^{(\infty)} = g^{\text{phys}}$ independent of $g^{(0)}$.
  \end{enumerate}

\paragraph{Seed-independence criterion.}
  The theory is consistent only if:
  \begin{equation}
    \forall \, g_{\mu\nu}^{(0)}, \, g'^{(0)}_{\mu\nu} \quad \text{(seeds)}: \quad
    \lim_{n\to\infty} g^{(n)} = \lim_{n\to\infty} g'^{(n)} = g^{\text{phys}}
  \end{equation}

  If different seeds yield different limiting metrics, the emergence claim is falsified.

\paragraph{Conformal freedom test.}
  A minimal test is conformal invariance: starting from $g^{(0)} = \Omega^2 \eta$ for various conformal factors $\Omega$, the physical metric $g^{\text{phys}}$ must be identical, demonstrating that the seed's geometry is physically irrelevant.

\paragraph{Current status.}
  This bootstrap procedure has been verified analytically for homogeneous cosmological solutions
  (Section~\ref{subsec:physical-interpretation}) and spherically symmetric configurations (Section~\ref{subsec:monotonicity-and-arrow-of-time}).
  General inhomogeneous solutions require numerical implementation and remain a priority for future work.

\subsection{Emergent Metric Structure}
\label{subsec:emergent-metric-structure}
...
This metric reproduces standard relativistic effects such as time dilation and length scaling as consequences of
$\chi$ dynamics.
...
Regions of high curvature in $\chi$
correspond to localized energy concentrations and are identified with particle-like excitations.
...
Conversely, in high-density regimes, strong gradients in $\chi$
reproduce the phenomenology of curved spacetime and gravitational collapse.
...
$V_{\text{Soliton}}(\chi)$
. Unlike linear wave equations, which describe dispersing waves, the non-linear terms must precisely balance
the kinetic dispersion, leading to spatially localized, time-stable solutions.
...
\begin{enumerate}
  ...
  number** (or an equivalent conserved quantity related to $\chi$'s phase) is hypothesized to provide this structural
  \item stability, preventing the soliton from decaying into the vacuum state $\chi \to 0$.
  \item \textbf{Emergence of Spin:} The explicit inclusion of Torsion in the full action (via
  $\mathcal{L}_{\text{Dirac}}^{\text{Torsion}}$
  ) ensures that these localized solutions carry the specific topological phase constraint required for fermionic
...
\subsubsection{Non-linear Resolution of the Hubble Tension}
  ...
  \paragraph{Local Variation and the Hubble Tension}
    ...
    This linear scaling constitutes the minimal closure relation between matter inhomogeneities and χ-field gradients, sufficient to capture the leading non-linear effect.
