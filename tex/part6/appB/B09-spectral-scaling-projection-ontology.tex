\subsection{Spectral Scaling and the Projection Ontology}
  ...
  \paragraph{Mass as Spectral Weight}
    The non-injective nature of the projection $\Pi$ (see Section~\ref{subsubsec:intrinsic-structural-indeterminacy})
    ...
    This ``fiber weight'' interpretation is diagnostic: it does not claim a one-to-one equality between eigenvalue
    magnitude and microscopic degeneracy, but asserts that both track the same coarse-grained constraint---
    projectability
    under $\Pi$ and resistance to relaxation.
  ...
  \paragraph{Invariance of the Ratio}
    ...
    This is consistent with the observed universality of the proton-to-electron mass ratio
    to the extent that it is observationally constrained to be stable, regardless of the global relaxation state of
$\chi$.

  \paragraph{The Spectral Packing Fraction (\texorpdfstring{$\alpha$}{α})}
    ...
    \begin{equation}
      ...
      \;\approx\; 3 \times 10^{-7}.
    \end{equation}
    While introduced here via an empirical target, $\alpha$ represents the ratio of spectral transmittance under $\Pi$.
    ...
    This topological constraint, often heuristically represented (as one possible representative) by a trefoil-like knot
    configuration, leads to a strong spectral gap between the binding modes $\lambda_{\mathrm{bind}}$ and the
fundamental
    electronic mode $\lambda_e$.

  \paragraph{Conclusion}
    While the precise numerical value $m_p/m_e \simeq 1836$ awaits a closed-form
    derivation from the spectral geometry of $L_{\mathrm{sol}}$ on topologically constrained effective
    manifolds / effective projected geometries, the Cosmochrony framework reformulates the mass-ratio
