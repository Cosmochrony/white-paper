...
\texorpdfstring{$\chi$}{χ} Relaxation}
  ...
  This appendix provides a detailed phenomenological treatment of the low-$\ell$
  CMB power suppression discussed in
  Sections~\ref{subsec:cmb} and~\ref{subsec:large-angle-anomalies}.
  ...
  \paragraph{Illustrative comparison with observations.}
    ...
    \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.85\textwidth]{part6/appC/C01-cmb_lowell_unsmoothed}
      \caption{
  ...
  \paragraph{Indicative low-\texorpdfstring{$\ell$}{ℓ} characterization.}
  ...
  \paragraph{Conceptual distinction from \texorpdfstring{$\Lambda$}{Λ}CDM.}
  ...
  \paragraph{Graph-theoretic origin of the admissibility filter.}
    Building on the concept of ontological poverty introduced in
    Section~\ref{subsec:initial-conditions-and-global-structure}, the admissibility
    filter $\mathcal{A}(k,t)$ admits a natural graph-theoretic interpretation.
    At any effective cosmological time $t$, the relational substrate $\chi$ defines
    a connectivity graph $G(t)$ whose weighted Laplacian $L_G(t)$ encodes the
    admissible collective modes.

    The spectrum of $L_G(t)$,
    \[
      L_G(t)\,\psi_n = \lambda_n(t)\,\psi_n,
    \]
    sets a minimal nonzero eigenvalue $\lambda_2(t)$ that characterizes the global
    connectivity of the relational structure.
    Modes with wavelengths exceeding the corresponding scale
    $k_c(t) \sim \sqrt{\lambda_2(t)}$ are structurally inadmissible.

    The primordial power spectrum is therefore modulated by a structural
    ...
    \[
      ...
      \right],
    \]
    reflecting the inability of an insufficiently connected relational graph to
    support large-scale coherent modes.
