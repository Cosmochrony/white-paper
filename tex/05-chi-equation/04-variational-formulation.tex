\subsection{Variational Formulation and Born--Infeld Action}
  ...
  \end{equation}
  where $\chi_{\mathrm{eff}}$ denotes the effective scalar descriptor introduced in Sec.~\ref{subsec:geometric-action},
  ...
  represents the effective density of localized, relaxation-resistant projected configurations.
  \begin{quote}
  \textit{Remark.}
  Unlike scalar-tensor theories (Section~\ref{subsec:not-scalar-tensor}), this action is \textbf{not fundamental} but an
  auxiliary encoding of projection constraints.
  ...
  \paragraph{Connection to Emergent Geometry.}
    ...
    The effective Lagrangian $\mathcal{L}_{\text{eff}}$
    (Eq.~\ref{eq:A12-effective-lagrangian_v171}) is not a fundamental action principle,
    but an \textbf{auxiliary representation} of the bounded-gradient constraint governing
    admissible $\chi$ configurations.
    It provides a compact encoding of the structural limitation on relational ordering,
    rather than a primitive dynamical postulate.

    In regimes where a geometric description is applicable, the Hessian of
    $\mathcal{L}_{\text{eff}}$ defines an emergent metric tensor
    $g_{\mu\nu}$ (Eq.~\ref{eq:A12-emergent-metric_v171}),
    which summarizes the effective geometric regularities of projected
    $\chi$ configurations.
    This metric inherits its validity and limitations entirely from the
    projective regime in which the effective description remains meaningful.
