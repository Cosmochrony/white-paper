\subsection{Relation to the Higgs Mechanism: Emergence from \texorpdfstring{$\chi$}{χ} Dynamics}
  ...
  independent scalar sector, but from the stabilization of locally
  stable projected configurations that saturate projective capacity.
  ...
  \subsubsection*{Structural Transition}
    ...
    Above~$\chi_c$ (structured regime), localized projected configurations
    with high spectral stability emerge as massive excitations.
    ...
    \end{equation}
    where $\chi_c$ has the status of a calibrated normalisation rather than a derived scale:
    matching the observed electroweak scale fixes $\chi_c \sim 10^{-18}\,\mathrm{m}$,
    with the same epistemic status as the induced gravitational coupling.
  ...
  \subsubsection*{Mass Generation as Solitonic Stabilization}
    ...
    spectral eigenvalues of the admissibility structure.
    The scope of this identification is delimited by the fermionic-matter
    sub-programme~\cite{Beau2026fmnote}.
    The spectral organization fixes the number of generations, three, through the
    stratigraphic selection of the binary icosahedral level structure~\cite{Beau2026a4},
    together with an order-one splitting of the squared Yukawa levels~\cite{Beau2026pyo}.
    It does not supply the observed inter-generation mass hierarchy:
    the direct identification of spectral levels with squared generation masses is
    excluded, and the hierarchy is carried neither by the order-one splitting nor by
    the single-level cascade rate~\cite{Beau2026pyo,Beau2026a5}.
    No independent mass-generating dynamics is introduced;
    the structural carrier of the hierarchy is an open problem, bounded by these
    no-go results.
