\subsection{Projective Non-Termination and the Condition of Temporal Ordering}
  ...
  Time is identified with the ordering of distinguishable projected states: $\Delta \tau \sim \mathrm{dist}(U_n, U_{n+1})$.
  A hypothetical terminal state~$U_0$ satisfying $\Pi(\chi + \delta\chi) = U_0$ for all admissible~$\delta\chi$ would
  ...
  bandwidth required to prevent collapse of the observable description into a stationary state.
  Time ceases only when the projection becomes stationary---a regime
  structurally forbidden by the existence of a finite bound on projective resolvability.

  The connection between projective stationarity and injectivity can now be made explicit.
  ...
  the inaccessibility of absolute zero (Section~\ref{subsec:structural-predictions-fluctuations}),
  the suppression of vacuum fluctuations in injective regimes (Section~\ref{subsec:projective-origin-of-fluctuations}),
  and the projective non-termination condition stated here.
  ...
  decohered regimes---but it cannot be globally reached without eliminating the effective description itself.
