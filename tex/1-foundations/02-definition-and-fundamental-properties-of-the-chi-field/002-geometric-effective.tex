\subsection{Effective Descriptions from Projective Structure}
  ...
  \texorpdfstring{$\chi$}{χ} Correlations}
    ...
    \paragraph{Effective scalar descriptor.}
      ...
      description.
      It does not correspond to the underlying relational substrate itself.

      Operational time intervals are defined from the accumulated ordering of
      projected configurations along admissible paths:
    \begin{equation}
      \tau_{AB} \;\propto\;
      \int_{\gamma_{AB}}
      ...
      ordering functional of the projected description and $\lambda$ an
      admissible ordering parameter.
    ...
    \node[box] (chi1)
    {$\chi_1$\\\footnotesize infra-physical\\
    \footnotesize relational substrate};
    \node[box, right=20mm of chi1] (chi2)
    {$\chi_2$\\\footnotesize infra-physical\\
    \footnotesize relational substrate};
    \node[box, below=of $(chi1)!0.5!(chi2)$] (chieff)
    {$\chi_{\mathrm{eff}}$\\\footnotesize effective physical\\
    \footnotesize regime\\\footnotesize (projectable regime)};
    \node[box, below=of chieff] (obs)
    {Observables\\\footnotesize context-dependent\\
    \footnotesize operational quantities};
    \draw[arrow] (chi1) -- node[left=2mm, note]
      {infra-physical\\projection $\pi$} (chieff);
    \draw[arrow] (chi2) -- node[right=2mm, note]
    ...
    \node[
      ...
      \caption{%
        ...
        substrate to an effective physical regime~$\chi_{\mathrm{eff}}$ in
        projectable regimes, generally in a non-injective manner (multiple
        infra-physical configurations can correspond to the same projected
        description).
        Physical observables arise from an operational projection~$\mathcal{O}$
        that specifies how $\chi_{\mathrm{eff}}$ is accessed under measurement
        contexts.}
  ...
  \subsubsection*{Effective Metric as a Descriptive Tool}
    ...
    Such smooth regimes arise when the projected spectrum is dominated by
    low-eigenvalue modes of the relational operator, ensuring slowly varying
    correlations and enabling a differentiable metric description to emerge.
  ...
  \subsubsection*{Consistency with General Relativity}
    ...
    curvature as emergent consequences of locally constrained projective ordering;
    homogeneous projected expansion yields an effective Hubble-like expansion law.
    All predictive content resides in the underlying projective structure.
