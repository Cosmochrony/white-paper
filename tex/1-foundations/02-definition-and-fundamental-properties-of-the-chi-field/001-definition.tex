\subsection{Definition of the \texorpdfstring{$\chi$}{χ} Field}
  ...
  The admissibility structure intrinsic to~$\chi$ orients, upon projection,
  the sequence of constrained reconstructions that is operationally perceived
  as temporal flow.

  Temporal ordering and spatial separation are not fundamental primitives but
  arise respectively from the ordering of admissible reprojections constrained
  by~$\chi$, and from their relational structure once a quasi-stable geometric
  regime is reached.
  ...
  emerge as harmonics of a single irreversible reprojection sequence.
  ...
  \begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=1.1]
      ...
      % Annotation
      \node[blue] at (4.5,2.4) {effective $\chi$ reprojection};
    \end{tikzpicture}
    ...
    The monotonic ordering of the effective reprojection sequence gives rise
