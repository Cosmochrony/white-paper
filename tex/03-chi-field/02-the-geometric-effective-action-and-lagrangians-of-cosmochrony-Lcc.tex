\subsection{The Geometric Effective Description of \texorpdfstring{$\chi$}{χ} Dynamics}
  \label{subsec:geometric-action}

  \subsubsection*{Effective Observables from \texorpdfstring{$\chi$}{χ} Correlations}
  ...
  Operational time intervals are defined from the accumulated ordering of projected
    ...
    % Dashed box: factorizable regime
    \node[
      draw,
      dashed,
      rounded corners,
      ...
      \caption{
        ...
        The infra-physical projection $\pi$ maps the fundamental $\chi$ substrate to an effective
        physical reality $\chi_{\mathrm{eff}}$ in projectable regimes, generally in a non-injective manner.
        Physical observables arise from a second, operational projection $\mathcal{O}$ that specifies
        how $\chi_{\mathrm{eff}}$ is accessed under measurement contexts.
        The dashed box indicates the factorizable regime where standard classical and relativistic
        descriptions are recovered.
      }
  ...
  \subsubsection*{Effective Metric as a Descriptive Tool}
    ...
    The effective metric is \textbf{not a fundamental geometric structure}, but a
    \textbf{derived descriptor} that summarizes relational correlations of projected
    $\chi$ configurations.
    Accordingly, it should not be interpreted as defining an underlying spacetime manifold,
    nor as an independent degree of freedom.

    The notation emphasizes that the metric characterizes correlations of effective
    descriptors, not properties of the fundamental $\chi$ substrate.
    It is introduced neither as a primitive object nor as a dynamical field, but as a
    post-hoc parametrization of effective relational regularities in regimes where a
    geometric description remains approximately valid.
    ...
    as such a coarse-grained description remains stable.
    For example:
    \begin{itemize}
    ...
    phenomena) arises from non-linear and threshold effects in $\chi$ dynamics and projection,
    not from an underlying discretization.
