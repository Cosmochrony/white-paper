\subsection{The Geometric Effective Description of \texorpdfstring{$\chi$}{χ} Dynamics}
  ...
  Operational time intervals are defined from the accumulated ordering of projected
    ...
    \draw[arrow] (chieff) -- node[right=2mm, note] {operational\\projection $\mathcal{O}$\\\footnotesize (contextual access)} (obs);
      ...
      \end{tikzpicture}
      \caption{Ontological pipeline in Cosmochrony. The infra-physical projection $\pi$ maps the fundamental substrate $\chi$ to an effective physical reality $\chi_{\mathrm{eff}}$ in projectable regimes (generally non-injective). Physical observables arise from a second, operational projection $\mathcal{O}$ that specifies how $\chi_{\mathrm{eff}}$ is accessed under measurement contexts, without introducing additional ontological structure.}
