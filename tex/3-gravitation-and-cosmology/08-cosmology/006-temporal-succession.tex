\subsection{The Self-Referential Origin of Temporal Succession}
  ...
  \paragraph{Two levels: structural rules and their execution.}
    ...
    the saturation bound~$c_{\mathrm{BI}}$, the admissibility envelopes, the fiber
  ...
  \paragraph{The ordering parameter $\tau$ and the relational energy.}
    ...
    once the hyperbolic signature of the effective operator is selected under
    the conditional analysis of~\cite{Beau2026c}.
  ...
  \paragraph{The universe state $U$ as its own cursor.}
    ...
    \begin{itemize}
      ...
      \item Among these constraints, the bounded-flux condition~$c_{\mathrm{BI}}$ imposes that
  ...
  \paragraph{Irreversibility and the arrow of time.}
    ...
    Within the present framework, this question admits a structural answer.
    A state~$U_0$ that were already a global minimum would require a
    projection~$\Pi_0$ with trivial kernel: every substrate configuration
    projecting to~$U_0$ would be uniquely identified, so that no residual
    fibre structure could drive the cascade forward.
    This condition is however structurally forbidden: the non-commutativity of
    the admissible fibre generators, $[X,\sigma(X)] = Z \neq 0$, guarantees
    that $\ker\Pi_0 \neq \{0\}$ for every admissible projection~\cite{Beau2026m}.
    The initial state therefore cannot be a global minimum of~$E_\chi$; the
    first transition is a structural necessity, not an initial condition to be
    separately imposed.
  ...
  \paragraph{Co-emergence of space and time from a single operator.}
    ...
    Following the conditional analysis of~\cite{Beau2026c}, given a second-order effective sector,
    the bounded-flux condition~$c_{\mathrm{BI}}$ constrains the admissible forms of the
    ...
    \begin{itemize}
      ...
      instantaneous propagation, violating~$c_{\mathrm{BI}}$.
  ...
  \paragraph{Metric rigidity and the general-relativistic space--time budget.}
    ...
    of flux conservation under the constrained effective operator~(\cite{Beau2026c}
    and~\cite[Sec.~5.3, 6]{Beau2026h}).

    The logical chain is therefore the following.
    Given a second-order effective sector, the bounded-flux condition~$c_{\mathrm{BI}}$ selects
    the $(-+++)$ signature, guaranteeing the existence of a pseudo-Riemannian metric and the
    universality of~$c$~(special relativity level).
  ...
  \paragraph{Summary.}
    ...
    \begin{enumerate}
      ...
      bound~$c_{\mathrm{BI}}$ are fixed and atemporal; they form the structural layer.
      ...
      satisfies~$c_{\mathrm{BI}}$; the constraint itself acts as the selection mechanism,
    ...
    developed in~\cite{Beau2026c, Beau2026h}, and with the relational fluctuation structure
