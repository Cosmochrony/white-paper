\subsection{Projective Cascade, Dynamical $\Lambda_{\mathrm{eff}}$,
  and the Cosmological Origin of Fundamental Constants}
  \label{sec:relax-cascade}

  The preceding sections establish $\chi(t)$ as the global effective projective field
  and $H(t) = \dot{\chi}/\chi$ as the emergent Hubble rate (Section~\ref{subsec:emergent-hubble-law},
  Eq.~\eqref{eq:hubble-like}).
  ...
  Eqs.~\eqref{eq:hbar_chi_def} and~\eqref{eq:K0chic_gravity}:
  \begin{enumerate}
    \item the effective cosmological constant $\Lambda_{\mathrm{eff}}$ is
    not a fundamental constant but a dynamical residual of ongoing projective cascade evolution,
    with equation of state $w \neq -1$;
    \item the projective cascade proceeds as a discrete sequence of projectable
    ...
    \item Planck's constant $\hbar$ is an epoch-dependent invariant of the current projective cascade epoch,
    fixed by $K_0$ and $\chi_c$ through Eq.~\eqref{eq:hbar_chi_def};
  ...
  \subsubsection*{Dynamical $\Lambda_{\mathrm{eff}}$ from Residual Projective Capacity}
    \label{sec:relax-cascade-lambda}

    \paragraph{Definition of residual projective capacity.}
      Energy is defined in this framework as the operational cost of modifying
      a projected configuration while preserving admissibility (\ref{subsec:key-conceptual-terms}).
      At the global cosmological level, the outstanding projective capacity
      of $\chi$ at cosmic time $t$ is the quantity that remains to be
      exhausted before the cascade reaches an asymptotically saturated state.
      We define it as a dimensionless fractional quantity $\Omega_\chi(t)$
      already introduced in Eq.~\eqref{eq:chi-projective-budget}:
    \begin{equation}
      \label{eq:rc-omega}
      ...
      \beta \;\equiv\; \frac{|\nabla\chi|}{c},
      \end{equation}
      which measures the fraction of projective capacity stored in active
      spatial gradients.
      The effective cosmological density associated with residual large-scale
      projective capacity (\ref{subsec:constants-and-emergent-limits}) is then
    \begin{equation}
    ...
    \begin{equation}
      ...
      (Section~\ref{sec:spectral-stability}) is the appropriate discriminating observable.
    ...
    \paragraph{Resolution of the coincidence problem.}
      ...
      Here, both $\rho_\Lambda \propto H^2$ and $\rho_m \propto H^2$, so their ratio
      $\rho_\Lambda / \rho_m \sim \Omega_\chi$ remains of order unity
      throughout the cascade-dominated epoch.
      The coincidence is therefore a structural consequence of the single
      projective cascade driving both matter inertia and residual vacuum energy.

  \subsubsection*{Discrete Projective Cascade and the Energy Hierarchy}
    \label{sec:relax-cascade-hier}

    \paragraph{Discreteness of projection.}
      The projectability threshold $\Theta_p$ (Appendix~\ref{subsec:chi_eff-validation}) defines the
      minimal spectral gap above which a configuration of $\chi$ admits a stable geometric description.
      Each time the relaxation network crosses $\Theta_p$ from above, a new
      projectable regime opens: this is what we call a \emph{relaxation step} $n \to n+1$.
      The minimum action exchanged per step is bounded below by the projection granularity
    \begin{equation}
      \label{eq:rc-delta-S}
      ...
      \frac{c^3}{K_0\,\chi_c},
      \end{equation}
      which is Eq.~\eqref{eq:hbar_chi_def}.
    ...
    \begin{equation}
      \label{eq:relational-energy}
      E_\chi[\varphi] \;\equiv\; \|\mathcal{L}\,\varphi\|^2
      \;=\; \sum_{i,j} w_{ij}(\varphi_i - \varphi_j)^2,
      \end{equation}
      measures the outstanding relaxation capacity at the microscopic level.
      Each relaxation step $n \to n+1$ corresponds to a monotone decrease of
      $E_\chi$, with the macroscopic counterpart $\Omega_\chi = \langle\beta^2\rangle$
      tracking the same quantity at the cosmological scale
      (Eq.~\eqref{eq:rc-omega}).

    \paragraph{Cascade law.}
      Let $E_n$ denote the effective energy available to the projected description at step $n$.
      Since each step irreversibly increases the number of distinguishable
      projected configurations $\Omega_n$, the energy per configuration decreases.
      ...
      self-similar spectral structure established in Appendix~\ref{subsec:chi-parameters}
      (the ratio $G_\mathrm{eff} \propto K_0/\chi_c^2$ is preserved under renormalisation), one obtains
    \begin{equation}
      \label{eq:rc-cascade}
      E_n \;\sim\; \frac{E_P}{n},
      \end{equation}
      ...
      (Section~\ref{subsec:big-bang-maximal-constraint}).
    ...
    \begin{equation}
      ...
      constant (the dark energy quantum $\hbar H_0$), consistently with Eq.~\eqref{eq:rc-rho-num}.
    ...
    \begin{equation}
      ...
      relaxation cascade, not an unexplained fine-tuning of a Higgs potential.
    ...
    \paragraph{Primordial energy and cascade consistency.}
      ...
      each cell the energy $E_P/N \sim 10^{-52}~\mathrm{J}$ of the current cascade step.
      ...
      most cells have relaxed to states with $n \gg N$ and contribute negligibly.
      A quantitative account requires the full spectral distribution of
      stabilisation depths across the relaxation network, which is left for future work.
  ...
  \subsubsection*{Planck's Constant as a Cascade-Epoch Invariant}
    ...
    \begin{equation}
      ...
      \frac{c^3}{K_0\,\chi_c},
      \end{equation}
      where $K_0$ is the maximal projective conductivity and $\chi_c$ is the
      critical relaxation threshold (Appendix~\ref{subsec:fundamental-quantities}).
      The identification $\hbar_\chi \simeq \hbar$ is operationally enforced
      in projectable regimes (Appendix~\ref{subsec:renormalization-hbar}, Eq.~\eqref{eq:hbar-chi}).

    \paragraph{Cosmological normalisation.}
      Appendix~\ref{subsec:chi-parameters} establishes that the dynamics are spectrally self-similar:
      $G$ remains constant under renormalisation provided the ratio $K_0/\chi_c^2$ is preserved.
      The cosmological-scale identification $\chi_c \sim c/H_0 = \chi(t_0)$
      (Appendix~\ref{subsec:observational-estimates}) then gives, using Eq.~\eqref{eq:K0chic_gravity},
    \begin{equation}
      ...
      \end{equation}
      which is the cosmological-scale entry of Table~\ref{tab:chi-parameters}.
    ...
    \begin{equation}
      ...
      Both branches are internally consistent, as stated in Appendix~\ref{subsec:chi-parameters}.
    ...
    \paragraph{Interpretation.}
      ...
      it characterises the global projective flux, not the granularity of
      ...
      (Section~\ref{subsec:local-slowdown-of-chi-relaxation}), $\chi_c$ may differ from its present-epoch value,
      causing both $\hbar$ and the fine-structure constant $\alpha$ to scale
      accordingly (Eq.~(9)), while leaving the product $K_0\,\chi_c^2 \propto G^{-1}$ invariant.

  \subsubsection*{The Big Bang as the First Globally Projectable Configuration}
    \label{sec:relax-cascade-bigbang}

    \paragraph{Projectability condition.}
      A configuration of $\chi$ is projectable if and only if its local Born--Infeld saturation parameter
      (Appendix~\ref{subsec:chi_eff-validation}, Eq.~\eqref{eq:D4_Si_Kij})
      satisfies
    \begin{equation}
      ...
      The network is in the maximally constrained regime of Section~\ref{subsec:big-bang-maximal-constraint}.
    ...
    \begin{equation}
      ...
      $L_G$ of the projective admissibility network (Appendix~\ref{app:lowell_attenuation}).
      The projectability threshold $\Theta_p$ defined in Appendix~\ref{subsec:chi_eff-validation} is
    ...
    \paragraph{Energy at first projection.}
      ...
      projectable modes is of order unity (by definition of the first crossing of $\Theta_p$).
    ...
    \begin{equation}
      ...
      then accounts for the reduction from $\rho_{\mathrm proj}$ to the present-day densities.

    \paragraph{Homogeneity as a structural consequence.}
      Prior to $t_{\mathrm proj}$, all configurations of $\chi$ belong to the
      same fibre of the (not yet defined) projection: they are operationally indistinguishable.
      ...
      $t_{\mathrm proj}$; the pre-projection substrate is globally connected by construction.
    ...
    \begin{table}[h]
      \centering
      \caption{Summary of the four structural results derived in Section~\ref{sec:relax-cascade}.
      Each result is a consequence of existing Cosmochrony equations; no new free parameter is introduced.}
      ...
      \begin{tabular}{lll}
        ...
        & Eqs.~(67), \eqref{eq:chi-projective-budget}, \eqref{eq:rc-rho-lambda} \\
        Energy hierarchy $E_P/E_n \sim n$
        & $E_n \sim E_P/n$
        & Eqs.~\eqref{eq:hbar_chi_def}, \eqref{eq:rc-cascade} \\
        $\hbar$ as epoch-dependent invariant
        & $\hbar_\chi = c^3/(K_0\chi_c)$
        & Eqs.~\eqref{eq:hbar_chi_def}, \eqref{eq:K0chic_gravity}, \eqref{eq:rc-hbar-cosmo} \\
