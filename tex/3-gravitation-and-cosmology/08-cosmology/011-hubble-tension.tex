\subsection{The Hubble Tension}
  ...
  The tension arises from using a single spacetime-based parametrization
  to describe observations sampling distinct stages of the projective cascade.
  ...
  Decoherence}
    ...
    \end{equation}
    where $\tau_\chi$ is an effective projective ordering timescale.
    Regions of high structural complexity (clusters, filaments) in which the projective
    ordering timescale acquires a spatial dependence:
    \begin{equation}
      \tau_\chi(\mathbf{x})
      = \tau_\chi^{(0)}
